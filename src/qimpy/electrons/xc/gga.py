...
class SpinInterpolated(Functional):
    ...
    def __call__(self, n: torch.Tensor, sigma: torch.Tensor,
        ...
        if n_spins == 2:
            ...
            sigma_tot = sigma[0]
        t2 = ((np.pi/96)**(2./3)) * sigma_tot * rs / (g * n_tot).square()
...
def PW91_H0(gamma: float, beta: float, g3: torch.Tensor, t2: torch.Tensor,
            ec_unif: torch.Tensor) -> torch.Tensor:
    ...
    frac = (1. + At2) / (1. + At2*(1. + At2))
    return gamma * g3 * (1. + beta_by_gamma * t2 * frac).log()
