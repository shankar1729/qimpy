...
__all__ = ["KE_TF", "X_Slater", "SpinInterpolated", "C_PZ", "C_PW", "C_VWN", "XC_Teter"]
...
class SpinInterpolated(Functional):
    ...
    def get_ec(self, rs: torch.Tensor, zeta: torch.Tensor) -> torch.Tensor:
        ...
        spin_polarized = (len(zeta) != 1) or (zeta.item() != 0.0)
...
class C_PZ(SpinInterpolated):
    ...
    def compute(self, rs: torch.Tensor, spin_polarized: bool) -> torch.Tensor:
        ...
        if len(sel[0]):
            rs_sel = rs[sel][..., None]  # add dim for spin interpolation
            e[sel] = gamma / (1.0 + beta1 * rs_sel.sqrt() + beta2 * rs_sel)
        return e
...
class C_PW(SpinInterpolated):
    ...
    def __init__(
        self, high_precision: bool, scale_factor: float = 1.0, helper: bool = False
    ) -> None:
        ...
        self._params[0] *= 2.0  # Convert A to 2A used below
    ...
    def compute(self, rs: torch.Tensor, spin_polarized: bool) -> torch.Tensor:
        ...
        return -A2 * (1 + alpha * rs[..., None]) * (1.0 + 1.0 / den).log()
