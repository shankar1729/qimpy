...
def _lbfgs(self: qp.utils.Minimize[Vector]) -> qp.Energy:
    ...
    gamma = 0.  # current scaling factor, updated each iteration
    ...
    for i_iter in range(self.n_iterations + 1):
        ...
        # Line minimization:
        step_size_test = min(self.step_size.initial,
                             self.safe_step_size(direction))
        ...
        history.append(_HistoryEntry(s=direction, Ky=Ky, rho=1./y_s))
