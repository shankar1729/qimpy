        ...
        for bk in range(n_bands):
            ...
            for iq, q in enumerate(kpoints.k):
                kernel = ExchangeSpherical_calc(
                    Rc, (q - k + G), grid.lattice.Gbasis, sym_tol
                ...
                for bq in range(n_bands):
                    ...
                    grad_Ipsiq = ~FieldC(
                        grid, data=prefac * fbk * Kn.data[:, None] * Ipsik * Nk
                    )  # *Nk ??
                    HC.coeff[:, iq, bq] += grad_Ipsiq.data.flatten(-3)[
                        :, :, fft_index[iq]
                    ]
...
def ExchangeSpherical_calc(
    Rc: float, k: torch.Tensor, Gbasis: torch.Tensor, tol: float
):
...
def main() -> None:
    input_dict = dict(
        checkpoint="/home/mkelley/RPI/Software/QimPy/tests/exchange/Si.h5"
    )
