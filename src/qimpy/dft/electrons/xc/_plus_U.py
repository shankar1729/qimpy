...
class PlusU(TreeNode):
    ...
    def _rhoAtom_getOrbitals(self, atom_index: int, orbital: str) -> torch.Tensor:
        ...
        atom_and_orbital_index = torch.logical_and(
            orbital_index[:, 0] == atom_index, orbital_index[:, 2] == ell
        )
    ...
    def _rhoAtom_calc(
        self, species: list[str], shells: list[str], C: Wavefunction, fillings: Fillings
    ) -> torch.Tensor:
    ...
    def rhoAtom_computeU(self, C: Wavefunction, fillings: Fillings) -> torch.Tensor:
        ...
        Utot = torch.einsum("a,amm -> ", Uprefac, (rhoAtom - rhoAtom @ rhoAtom).real)
        self.U_rho = torch.diag(Uprefac) - 2 * torch.einsum(
            "a,amn -> amn", Uprefac, rhoAtom
        )
        return Utot
    ...
    def rhoAtom_grad(self, C: Wavefunction) -> Wavefunction:
        ...
        for specie, orbital in zip(self.species, self.shells):
            ...
            for n_atoms in range(self.n_ions_type[ion_index]):
                ...
                gradient.coeff += torch.einsum(
                    "k, skbrn, abb, skmb -> skmrn",
                    1 / self.wk,
                    atom_and_orbitals.coeff,
                    self.U_rho,
                    psi_phi_inner,
                )
