...
def lbfgs(self: algorithms.Minimize[Vector]) -> Energy:
    ...
    for i_iter in range(self.i_iter_start, self.n_iterations + 1):
        ...
        for h in reversed(history):
            alpha_i = h.rho * self._sync(h.s.vdot(direction)).item()
        ...
        for h in history:
            alpha_i = alpha.pop()
            beta = h.rho * self._sync(h.Ky.vdot(direction)).item()
        ...
        y_s = self._sync(y.vdot(direction)).item()
        gamma = y_s / self._sync(y.vdot(Ky)).item()
