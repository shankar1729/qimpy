...
def main() -> None:
    ...
    ap.add_argument("--ridge", type=float, default=1e-13)
    ap.add_argument("--map", default="tanh", choices=["tanh", "linear"])
    ...
    for te in a.te:
        ...
        coef = torch.linalg.solve_triangular(
            R, (Q.T @ u_in).unsqueeze(-1), upper=True).squeeze(-1)
        for deg in a.angles:
            phi = np.deg2rad(deg)
