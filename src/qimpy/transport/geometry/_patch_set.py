...
from qimpy.transport.advection import VprimeParams
...
class PatchSet(Geometry):
    ...
    def apply_boundaries(self, rho_list: TensorList, t: float) -> TensorList:
        ...
        for out, rho in zip(out_list, rho_list):
            out[VprimeParams.NON_GHOST, VprimeParams.NON_GHOST] = rho
...
IN_SLICES = [
    (slice(None), VprimeParams.GHOST_L),
    (VprimeParams.GHOST_R, slice(None)),
    (slice(None), VprimeParams.GHOST_R),
    (VprimeParams.GHOST_L, slice(None)),
]  #: input slice for each edge orientation during edge communication

OUT_SLICES = [
    (VprimeParams.NON_GHOST, VprimeParams.GHOST_L),
    (VprimeParams.GHOST_R, VprimeParams.NON_GHOST),
    (VprimeParams.NON_GHOST, VprimeParams.GHOST_R),
    (VprimeParams.GHOST_L, VprimeParams.NON_GHOST),
