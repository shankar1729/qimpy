...
def test_closed_domain_conserves_mass_radial() -> None:
    ...
    ⛔ The Nr=1 test above cannot see a radial-basis defect: at Nr=1 the radial
    quadrature is the single trivial weight 1.0, so every per-mode (D, T) solve
    collapses to the scalar case and a wrong radial weighting is invisible."""
    torch.set_default_dtype(torch.float64)
    tmp = tempfile.mkdtemp()
    path = _make_rect_mesh(12.0, os.path.join(tmp, "rect.npz"), all_walls=True)
