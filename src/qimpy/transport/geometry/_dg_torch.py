...
class AdvectTorch:
    ...
    def _residual(self, u, ax, ay, t=0.0):
        ...
        fstar = adn * 0.5 * (uM + uP) + 0.5 * adn.abs() * (uM - uP)
        fstar = fstar.reshape(K, Nfaces * Nfp, C).permute(1, 0, 2)
        Fx = ax * u; Fy = ay * u
    ...
    def rhs(self, u, ax, ay, t=0.0):
        """d u/dt in the nodal density u (= M_k^{-1}(vol - surf))."""
        return self.apply_mass_inv(self._residual(u, ax, ay, t))

    def rhs_w(self, w, ax, ay, t=0.0):
        """d w/dt for the conservative variable w = M_k u (= vol - surf)."""
        return self._residual(self.apply_mass_inv(w), ax, ay, t)
