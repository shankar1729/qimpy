...
def main():
    ...
    for deriv in range(5):
        plt.plot(
            t.to(rc.cpu),
            qp.ions.quintic_spline.Interpolator(t, 2.0, deriv)(coeff).to(rc.cpu),
            label=f"Deriv: {deriv}",
        )
    ...
    # Generate test data:
    dx, x_fine, y_fine, y_prime_fine, y_coeff = test_interpolator(rc)

    # Plot results:
    plt.figure()
    plt.plot(
        x_fine.to(rc.cpu), y_fine.to(rc.cpu), "k--", label="Reference data", zorder=10
    )
    plt.plot(
        x_fine.to(rc.cpu),
    ...
    for deriv in range(5):
        plt.plot(
            x_fine.to(rc.cpu),
            qp.ions.quintic_spline.Interpolator(x_fine, dx, deriv)(y_coeff).to(rc.cpu),
