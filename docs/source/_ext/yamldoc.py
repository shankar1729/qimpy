...
class ClassInputDoc:
                    ...
                    for cls_option in get_args(param_type):
                        if inspect.isclass(cls_option) and issubclass(
                            ...
                            if param_class is None:
                                param_class = ClassInputDoc(
                                    cls_option, constructable, app, classdocs, outdir
                                )
    ...
    def write_rest_file(self, fname: str) -> None:
        ...
        with open(fname, "w") as fp:
            ...
            # Component classes:
            component_classdocs = [
                param.classdoc for param in self.params if (param.classdoc is not None)
            ]
            if component_classdocs:
                write_title(fp, "Component classes:", "-")
                fp.write(".. toctree::\n    :maxdepth: 1\n\n")
                for classdoc in component_classdocs:
                    fp.write(f"    {classdoc.cls.__qualname__}" f" <{classdoc.path}>\n")
                fp.write("\n")
            ...
            for param in self.params:
                ...
                if param.classdoc is None:
                    ...
                    fp.write(
                        f"*Type:* :doc:`{param.classdoc.cls.__qualname__}"
    ...
    def get_yaml_template(self, linkprefix="") -> list[str]:
        ...
        for param in self.params:
            ...
            value = (
                (param.default if param.default else f" [{param.typename}]")
                if (param.classdoc is None)
                else ""
            )  # don't put value if class doc follows
...
def create_yamldoc_rst_files(app: Sphinx) -> None:
    ...
    docfiles = [
        ...
        if (
            (not docname.startswith(("api/", "yamldoc/")))
    ...
    print(
        "[yamldoc] generating input documentation for root class(es):",
...
YAML_TYPE = {
    ...
    get_class_from_path("qimpy.mpi.Unit"): ":yamltype:`Unit`",
}
...
def yaml_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    return [
        nodes.reference(rawtext, "[Input file]", refuri=uri, **options),
        nodes.Text(" " + text),
    ], []


def yaml_param_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    """Link :yamlparam: in the input file doc to detailed version."""
    env = inliner.document.settings.env
    app: Sphinx = env.app
    dest_doc, param_name = text.split(":")
    uri = (
        app.builder.get_relative_uri(env.docname, "yamldoc/" + dest_doc)
        + "#"
        + param_name
    )
    return [
        nodes.reference(
            rawtext, param_name, refuri=uri, classes=["yamlparam"], **options
        )
    ], []
